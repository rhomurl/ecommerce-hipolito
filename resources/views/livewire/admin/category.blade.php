<tr class="text-gray-700 dark:text-gray-400">
    ...
    <td class="px-4 py-3">
        <div class="flex items-center space-x-4 text-sm">
            ...
            <button wire:click="delete_dialog({{ $category->id }}, 'Category')" class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 hover:bg-gray-200 focus:outline-none focus:shadow-outline-gray" aria-label="Delete">
                <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
            ...
            {{--<div x-data="{ confirmDelete:false }" class="flex flex-wrap -mx-2 overflow-hidden">
                ...
                <div class="my-2 px-2 w-1/2 overflow-hidden">
                    <button x-show="confirmDelete" x-on:click="confirmDelete=false"  wire:click="confirmDelete({{ $category->id }})" class="flex items-center justify-between px-4 py-2 text-sm font-medium leading-5 text-white transition-colors duration-150 bg-blue-600 border border-transparent rounded-lg active:bg-green-600 hover:bg-green-700 focus:outline-none focus:shadow-outline-green">Yes</button>
                </div>
